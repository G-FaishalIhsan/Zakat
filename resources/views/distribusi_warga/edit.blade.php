...
@section('title', 'Edit Distribusi Zakat Fitrah Warga')
@section('content')
    <div class="col-mb-6">
        <div class="card">
            <h5 class="card-header">Edit Distribusi Zakat Fitrah Warga</h5>
            ...
            <form action="{{ route('distribusi-warga.update', $distribusi->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="mb-4">
                        ...
                        <select name="nama_mustahik" id="nama_mustahik" class="form-control" required>
                            ...
                            @foreach ($mustahikList as $mustahik)
                                <option value="{{ $mustahik->nama_mustahik }}" data-hak="{{ $mustahik->jumlah_hak }}"
                                    {{ $distribusi->nama_mustahik == $mustahik->nama_mustahik ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="jenis_bayar" id="jenis_bayar" class="form-control" required>
                            ...
                            <option value="beras" {{ $distribusi->jumlah_beras ? 'selected' : '' }}>Beras</option>
                            <option value="uang" {{ $distribusi->jumlah_uang ? 'selected' : '' }}>Uang</option>
                        </select>
                    </div>

                    <div class="mb-4" id="beras-field" style="{{ $distribusi->jumlah_beras ? 'display: block;' : 'display: none;' }}">
                        <label class="form-label">Jumlah Beras (kg)</label>
                        <input type="number" name="jumlah_beras" id="jumlah_beras" class="form-control"
                            placeholder="Contoh: 2.5" step="0.1" min="2.5" value="{{ $distribusi->jumlah_beras ?? '' }}" />
                        <small class="text-muted">Standar zakat fitrah 2.5 kg per orang</small>
                    </div>

                    <div class="mb-4" id="uang-field" style="{{ $distribusi->jumlah_uang ? 'display: block;' : 'display: none;' }}">
                        <label class="form-label">Jumlah Uang (Rp)</label>
                        <input type="number" name="jumlah_uang" id="jumlah_uang" class="form-control"
                            placeholder="Contoh: 35000" step="1000" min="35000" value="{{ $distribusi->jumlah_uang ?? '' }}" />
                        <small class="text-muted">Standar zakat fitrah Rp35,000 per orang</small>
                    </div>

                    <div class="mb-4">
                        <button type="submit" class="btn btn-primary">Update Distribusi</button>
    ...
    <script>
        $(document).ready(function() {
            // Set initial jumlah hak
            var initialJumlahHak = $('#nama_mustahik').find(':selected').data('hak');
            $('#jumlah_hak').val(initialJumlahHak);
