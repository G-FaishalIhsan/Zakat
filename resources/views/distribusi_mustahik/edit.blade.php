...
@section('title', 'Edit Distribusi Zakat Fitrah Mustahik')
@section('content')
    <div class="col-mb-6">
        <div class="card">
            <h5 class="card-header">Edit Distribusi Zakat Fitrah Mustahik</h5>
            ...
            <form action="{{ route('distribusi-mustahik.update', $distribusi->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="jenis_bayar" id="jenis_bayar" class="form-control" required>
                            ...
                            <option value="beras" {{ $distribusi->jumlah_beras > 0 ? 'selected' : '' }}>Beras</option>
                            <option value="uang" {{ $distribusi->jumlah_uang > 0 ? 'selected' : '' }}>Uang</option>
                        </select>
                    </div>

                    <div class="mb-4" id="beras-field"
                        style="display: {{ $distribusi->jumlah_beras > 0 ? 'block' : 'none' }};">
                        ...
                        <input type="number" name="jumlah_beras" id="jumlah_beras" class="form-control"
                            ...
                            value="{{ $distribusi->jumlah_beras ?? '' }}" />
                        <small class="text-muted">Standar zakat fitrah 2.5 kg per orang</small>
                    </div>

                    <div class="mb-4" id="uang-field"
                        style="display: {{ $distribusi->jumlah_uang > 0 ? 'block' : 'none' }};">
                    ...
                    <div class="mb-4">
                        ...
                        <a href="{{ route('distribusi-mustahik.index') }}" class="btn btn-secondary">Kembali</a>
                    </div>
                </div>
            </form>
        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var selectedMustahik = $('#nama_mustahik').find(':selected');
            if (selectedMustahik.length) {
                var jumlahHak = selectedMustahik.data('hak');
